...
const FIREBASE_AUTH_DOMAIN = 'sigma-ai-co.firebaseapp.com';
const FIREBASE_PROJECT_ID = 'sigma-ai-co';
...
function getLoginHtml(port: number, state: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .btn {
      ...
      background: #fff; color: #1e293b; border: none; padding: 12px 24px;
...
<body>
  <div class="card">
    <h1>Sigma AI</h1>
    <p>Sign in to use the Sigma AI CLI</p>
    ...
    document.getElementById('googleBtn').onclick = async () => {
      ...
      try {
        ...
        const res = await fetch('/callback', {
          ...
          body: JSON.stringify({ idToken, refreshToken, email: user.email, state: '${state}' }),
        });
        if (res.ok) {
  ...
  const creds = await new Promise<Credentials>((resolve, reject) => {
    const server = createServer((req, res) => {
      if (req.method === 'GET' && req.url === '/') {
        ...
        res.end(getLoginHtml(port, state));
        return;
      }
